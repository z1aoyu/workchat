 ...
 * @description:部门内详细员工信息
 */
@Data
public class OrgUserDetailInfoBo {
    ...
    /**
     * 表示在所在的部门内是否为部门负责人。0-否；1-是。是一个列表，数量必须与department一致。第三方通讯录应用或者授权了“组织架构信息-应用可获取企业的部门组织架构信息-部门负责人”权限的第三方应用可获取；对于非第三方创建的成员，第三方未授权应用不可获取；上游企业不可获取下游企业成员该字段
     **/
    private List<String> is_leader_in_dept;

    /**
     * 直属上级UserID，最多有5个直属上级；第三方通讯录应用或者授权了“组织架构信息-应用可获取可见范围内成员组织架构信息-直属上级”权限的第三方应用可获取；对于非第三方创建的成员，第三方未授权应用不可获取；上游企业不可获取下游企业成员该字段
     **/
